...
import random
import asyncio
from logger_config import log_error, log_debug
...
class BrowserFingerprint:
    ...
    async def apply_fingerprint(page):
        ...
        try:
            ...
            log_error(f"❌ Error applying fingerprint: {e}")
            return False
...
class MouseBehavior:
    ...
    async def scroll_to_element(page, selector):
        ...
        try:
            ...
            log_error(f"❌ Error scrolling to element: {e}")
            return False
...
class TypingBehavior:
    ...
    def __init__(self, profile='variable'):
        ...
        self.min_delay, self.max_delay = self.TYPING_PROFILES.get(
            profile, self.TYPING_PROFILES['variable'])
        ...
        for i, char in enumerate(text):
            try:
                ...
                if random.random() < 0.05 and char.isalpha():
                    ...
                    if log_callback:
                        log_callback(
                            f"Typed wrong, fixed: {wrong_char} → {char}", is_technical=True)
                ...
                if random.random() < 0.03:
                    ...
                    if log_callback:
                        log_callback(
                            f"Thinking pause: {pause_time:.1f}s", is_technical=True)
                ...
                log_error(f"❌ Error typing character: {e}")
                return False
...
class AntiDetectionManager:
    ...
    async def type_post_content(self, page, selector, content):
        ...
        try:
            ...
            log_error(f"❌ Error typing content: {e}")
            return False
