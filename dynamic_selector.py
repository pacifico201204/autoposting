...
class DynamicSelector:
    ...
    async def find_post_input_box(self):
        ...
        selectors_to_try = [
            ...
            lambda: self.page.locator('input[placeholder*="Write"], input[placeholder*="Viết"]'),
            
            # Method 4: By aria-label
            lambda: self.page.locator('[aria-label*="Write"], [aria-label*="Viết"]'),
            
            # Method 5: By class pattern (Facebook-style)
            lambda: self.page.locator('div[role="button"] >> text=/Write something|Viết nội dung|Bạn đang nghĩ gì|Viết/i'),
        ...
        for idx, selector_func in enumerate(selectors_to_try, 1):
            try:
                ...
                if count > 0:
                    print(f"✓ Found post box using method {idx}")
                ...
                if "context" in error_str.lower() and "destroyed" in error_str.lower():
                    ...
                    print(f"✗ Method {idx} failed - context destroyed: {error_str[:50]}")
                    raise
                else:
                    print(f"✗ Method {idx} failed: {error_str[:50]}")
                continue
    ...
    async def find_post_button(self):
        ...
        selectors_to_try = [
            ...
            lambda: self.page.locator('div[role="dialog"] [aria-label*="Post"], [aria-label*="Đăng"]'),
            
            # Method 2: By text
            lambda: self.page.locator('div[role="dialog"] button:has-text("Post")'),
            lambda: self.page.locator('div[role="dialog"] button:has-text("Đăng")'),
            
            # Method 3: By class pattern
            lambda: self.page.locator('div[role="dialog"] button:has-text(/Post|Đăng|Share/i)'),
            
            # Method 4: By data attribute
            lambda: self.page.locator('button[data-testid*="post"], button[data-testid*="share"]'),
        ...
        for idx, selector_func in enumerate(selectors_to_try, 1):
            try:
                ...
                count = await locator.count()
                if count > 0:
                    print(f"✓ Found post button using method {idx}")
                    return locator.first if count > 1 else locator
            except Exception as e:
                error_str = str(e)
                if "context" in error_str.lower() and "destroyed" in error_str.lower():
    ...
    async def find_file_input(self):
        ...
        selectors_to_try = [
            ...
            lambda: self.page.locator('input[type="file"][style*="display: none"]'),
            
            # Method 4: By name attribute
            lambda: self.page.locator('input[name*="upload"], input[name*="file"], input[name*="image"]'),
        ]
        ...
        for idx, selector_func in enumerate(selectors_to_try, 1):
            try:
                ...
                count = await locator.count()
                if count > 0:
                    print(f"✓ Found file input using method {idx}")
    ...
    async def find_element_by_text(self, text_patterns: list, max_wait=5000):
        ...
        try:
            ...
            print(f"Text search failed: {str(e)[:50]}")
    ...
    async def wait_for_main_content(self, timeout=30000):
        ...
        try:
            ...
            if "context" in error_str.lower() and "destroyed" in error_str.lower():
                ...
                print(f"Context destroyed during wait_for_main_content: {error_str[:50]}")
                return False
            return False
    ...
    async def find_close_button_in_dialog(self):
        ...
        selectors_to_try = [
            # Method 1: aria-label
            lambda: self.page.locator('div[role="dialog"] [aria-label="Close"]'),
            
            # Method 2: SVG close icon
            lambda: self.page.locator('div[role="dialog"] button >> svg'),
            
            # Method 3: Text Close
            lambda: self.page.locator('div[role="dialog"] button:has-text("Close")'),
        ]
